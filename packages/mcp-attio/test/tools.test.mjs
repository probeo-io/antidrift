...
const EXPECTED_TOOLS = [
  'attio_list_people', 'attio_search_people', 'attio_get_person', 'attio_create_person',
  'attio_list_companies', 'attio_search_companies', 'attio_create_company',
  'attio_list_deals', 'attio_update_record', 'attio_move_deal',
  'attio_create_task', 'attio_list_tasks', 'attio_complete_task', 'attio_add_note',
];

describe('attio tool definitions', () => {
  ...
  it('all tool names start with attio_ prefix', () => {
    for (const tool of tools) {
      assert.ok(tool.name.startsWith('attio_'), `Tool ${tool.name} missing attio_ prefix`);
    }
  });
