...
function writeMcpConfig() {
  ...
  const serverDir = isGlobal
    ? join(homedir(), '.antidrift', 'tools', 'attio')
    : join(process.cwd(), '.mcp-servers', 'attio');
  ...
  if (targets.global) {
    ...
    zeroConfig.credentials['attio'] = { file: join(homedir(), '.antidrift', 'attio.json') };
  ...
  if (targets.cowork) {
    writeDesktopConfig('antidrift-attio', serverPath);
