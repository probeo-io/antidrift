...
function writeMcpConfig() {
  ...
  const serverDir = isGlobal
    ? join(homedir(), '.antidrift', 'tools', 'notion')
    : join(process.cwd(), '.mcp-servers', 'notion');
  ...
  if (targets.global) {
    ...
    zeroConfig.credentials['notion'] = { file: join(homedir(), '.antidrift', 'notion.json') };
  ...
  if (targets.cowork) {
    writeDesktopConfig('antidrift-notion', serverPath);
