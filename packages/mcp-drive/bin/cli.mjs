...
async function main() {
  ...
  if (command === 'add' || command === 'setup' || !command) {
    ...
    console.log(`
@antidrift/mcp-drive — Google Drive for your AI agent

Usage:
  antidrift connect drive               Connect Google Drive
  antidrift connect drive status        Check connection status
  antidrift connect drive reset         Clear credentials and re-authorize
`);
  }
}
...
`);
  ...
  if (existsSync(tokenPath)) {
    ...
    console.log('  ✓ Drive updated. Restart your agent to pick up changes.\n');
    process.exit(0);
  }
...
function status() {
  ...
  const icon = hasToken ? '✓' : '○';
  console.log(`\n  ${icon} Google Drive — ${hasToken ? 'connected' : 'not connected'}`);
  console.log('    Drive, Docs, Sheets\n');
}
...
async function writeMcpConfig() {
  ...
  const serverDir = isGlobal
    ? join(homedir(), '.antidrift', 'tools', 'drive')
    : join(process.cwd(), '.mcp-servers', 'drive');
  ...
  if (targets.global) {
    ...
    zeroConfig.credentials['drive'] = { file: join(homedir(), '.antidrift', 'google.json') };
    ...
    if (!config.mcpServers) config.mcpServers = {};
    config.mcpServers['antidrift-drive'] = {
      command: 'node',
      args: [join('.mcp-servers', 'drive', 'server.mjs')]
    };
  ...
  if (targets.cowork) {
    writeDesktopConfig('antidrift-drive', serverPath);
