...
const EXPECTED_TOOLS = [
  'github_user', 'github_list_repos', 'github_search_repos', 'github_get_repo',
  'github_list_issues', 'github_get_issue', 'github_create_issue',
  'github_list_prs', 'github_get_pr', 'github_pr_diff',
  'github_list_runs', 'github_get_file', 'github_list_branches',
  'github_list_releases', 'github_repo_traffic',
];
...
describe('github tool definitions', () => {
  ...
  it('all tool names start with github_ prefix', () => {
    for (const tool of tools) {
      assert.ok(tool.name.startsWith('github_'), `Tool ${tool.name} missing github_ prefix`);
    }
  });
