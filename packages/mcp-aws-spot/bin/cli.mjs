...
async function main() {
  ...
  if (command === 'add' || command === 'setup' || !command) {
    ...
    console.log('  AWS Spot uses your standard AWS credentials (~/.aws/credentials).');
    console.log('  Run: aws configure\n');
    rl.close();
    process.exit(0);
  } else {
    console.log(`
@antidrift/mcp-aws-spot — EC2 Spot pricing for your AI agent

Usage:
  antidrift connect aws-spot               Connect AWS Spot
  antidrift connect aws-spot status        Check connection status
`);
  }
...
`);
  ...
  writeMcpConfig();
  console.log('  ✓ AWS Spot connected (prices, AZ comparison, region comparison, placement scores)');
...
function status() {
  ...
  console.log(`\n  ${icon} AWS Spot — ${hasCredentials ? 'connected' : 'not connected'}`);
  console.log('    Spot prices, AZ comparison, region comparison, placement scores\n');
}
...
function writeMcpConfig() {
  ...
  const serverDir = isGlobal
    ? join(homedir(), '.antidrift', 'tools', 'aws-spot')
    : join(process.cwd(), '.mcp-servers', 'aws-spot');
  ...
  if (isGlobal) {
    ...
    writeFileSync(join(serverDir, 'package.json'), JSON.stringify({"type":"module","dependencies":{"@aws-sdk/client-ec2":"*"}}));
    console.log('    Installing @aws-sdk/client-ec2 dependencies...');
  ...
  if (targets.global) {
    ...
    zeroConfig.credentials['aws-spot'] = { file: join(homedir(), '.antidrift', 'aws-spot.json') };
    ...
    if (!config.mcpServers) config.mcpServers = {};
    config.mcpServers['antidrift-aws-spot'] = {
      command: 'node',
      args: [join('.mcp-servers', 'aws-spot', 'server.mjs')]
    };
  ...
  if (targets.cowork) {
    writeDesktopConfig('antidrift-aws-spot', serverPath);
