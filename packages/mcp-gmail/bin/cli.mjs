...
async function writeMcpConfig() {
  ...
  const serverDir = isGlobal
    ? join(homedir(), '.antidrift', 'tools', 'gmail')
    : join(process.cwd(), '.mcp-servers', 'gmail');
  ...
  if (targets.global) {
    ...
    zeroConfig.credentials['gmail'] = { file: join(homedir(), '.antidrift', 'google.json') };
  ...
  if (targets.cowork) {
    writeDesktopConfig('antidrift-gmail', serverPath);
